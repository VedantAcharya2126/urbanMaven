...
const bookService = (req, res) => {
    ...
    let data = common.sanitize(req.body, schemas.bookService);
    logger.info(`Save Rejection Details data: ${JSON.stringify(req.body)}`);
    if (schemas.validate(data, schemas.bookService)) {
        model.bookService(data).then((result) => {
            res.status(200).send({
                code: 2000,
